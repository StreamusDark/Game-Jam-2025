...
func initialise_signals():
	for btn in [cups_button, liquid_button, steam_button, $Content/Jug/MilkJug, $Content/Decorations/MilkBottle/Button, $Exit/Button, $UseLast/Button]:
...
func milk_jug_clicked() -> void:
	if (not milk_bottle_pouring) and (not steam_active):
		...
		milk_jug_options.get_node("Use").visible = (not coffee_pouring_active) and (current_cup_type in ["espresso","double_espresso"]) and milk_in_jug and (current_cup_in_deco == milk_in_deco)
...
func milk_move_up() -> void:
	...
	get_tree().create_tween().tween_property(milk_jug, "position", milk_jug_positons["up"], 0.2)

func milk_move_down() -> void:
	change_selectable("close")
	milk_in_deco = true
	
	get_tree().create_tween().tween_property(milk_jug, "position", milk_jug_positons["down"], 0.2)
